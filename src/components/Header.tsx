          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="hidden sm:block">
              <h1 className="text-lg font-serif font-bold text-gray-900">
                Lasc Camelia <span className="text-yellow-600">|</span> Cabinet Avocat
              </h1>
            </div>
          </div>
            ...
            <nav className="px-4 py-4 space-y-2">
              {menuItems.map((item, index) => (
                <button
                  key={index}
                  onClick={() => scrollToSection(item.href.substring(1))}
                  className="block w-full text-left px-4 py-3 text-gray-700 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg font-medium transition-all duration-200"
                >
                  {item.label}
                </button>
              ))}
